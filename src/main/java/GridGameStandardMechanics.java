...
public class GridGameStandardMechanics implements FullJointModel {
    ...
    /**
     * Mechanism 1: Handling potential collision of ja when at least one agent NOOP. This is the first check.
     * Mechanism 2: Agents move into each other's position, thus colliding: resolvePositionSwaps
    ...
    public State sample(State s, JointAction ja) {
        ...
        //Mechanism 1
        //need to force no movement when trying to enter space of a noop agent
        //Iterate through ja. Add to previousLocations. If NOOP, add to noopLocations.
        List <Location2> previousLocations = new ArrayList<GridGameStandardMechanics.Location2>();
        List <Location2> noopLocations = new ArrayList<GridGameStandardMechanics.Location2>();
        ...
        //Execute moves --> change agent positions based on finalPositions
    ...
    protected List<Location2> resolvePositionSwaps(List <Location2> originalPositions, List<Location2> desiredPositions, OOState s){
        ...
        for(int i = 0; i < originalPositions.size(); i++){
            ...
            for(int j = i+1; j < resolvedPositions.size(); j++){ //j = i+1
